...
|-----------|--------------|--------------------|------------------|--------------|-------|
| OBJ-001 | FR-001–FR-011; NFR-004–NFR-009 | US1, US2; EC-002–EC-006, EC-010, EC-015, EC-023–EC-024 | SC-001, SC-002 | Composite-state, command-result, and propagation scenarios | LL-US1-01–03; LL-US2-01–03 |
| OBJ-002 | FR-012–FR-021; NFR-008–NFR-009, NFR-014 | US3; EC-002–EC-003, EC-007–EC-010, EC-021, EC-024–EC-026 | SC-003 | Accounting boundary/property/concurrency scenarios | LL-US3-01–04 |
| OBJ-003 | FR-022–FR-030; NFR-002, NFR-010, NFR-014 | US4; EC-011–EC-015, EC-022 | SC-004 | Access/paging/data-quality/privacy scenarios | LL-US4-01–04 |
| OBJ-004 | FR-031–FR-037; NFR-001–NFR-002 | US5; EC-005, EC-016–EC-018 | SC-005 | Role/case/purpose matrix and manual review | LL-US5-01–03, LL-US5-05 |
| OBJ-005 | FR-005, FR-021, FR-037–FR-041; NFR-002–NFR-003, NFR-011–NFR-012 | All stories; EC-002–EC-026 | SC-006 | Audit recovery/reconciliation/taxonomy review | LL-US1-01–03, LL-US2-01–03, LL-US3-01–04, LL-US5-01–05 |
| OBJ-006 | FR-003, FR-006, FR-010, FR-016, FR-018–FR-020, FR-027–FR-030, FR-033, FR-041; NFR-003–NFR-014 | All stories; EC-002–EC-026 | SC-001, SC-003, SC-004, SC-007 | Performance/reliability/recovery and boundary scenarios | LL-US1-02–03, LL-US2-02–03, LL-US3-04, LL-US4-02–04, LL-US5-02, LL-US5-04–05 |
...
# Low-Level Future Implementation Tasks

This section is synchronized from the validated Spec Kit task slices. It describes future implementable work but does not authorize code for Homework 3.
...
## Normative Outcome-to-Evidence Reconciliation

This matrix makes the existing `OBJ-005`, `FR-005`, `FR-021`, `FR-037–FR-041`, `NFR-003`, and
`SC-006` intent deterministic. It does not turn retry delivery into a second business outcome.

| Outcome/activity | Business evidence rule |
|------------------|------------------------|
| State-changing `SUCCEEDED` | One root audit identity and durable intent share the acceptance boundary; immutable published evidence carries the sanitized before/after and result. |
| Distinct successful no-op | One attributable non-changing outcome under a new root identity; no state-transition or propagation event is fabricated. |
| `PENDING_PROPAGATION` | The accepted freeze has one root identity; initial pending and final confirmation/escalation are immutable linked events under that root. |
| `DENIED` / `VALIDATION_ERROR` / `CONFLICT` / `IDEMPOTENCY_MISMATCH` / `TEMPORARILY_UNAVAILABLE` | Each distinct command identity produces one sanitized non-changing outcome event with the approved safe category and no target/dependency leakage. |
| Exact bound retry | Returns and references the stored original outcome/root identity; no second business event or transition is created. Sanitized delivery diagnostics may record transport timing only. |
| Privileged search/read/action/export | Every allowed or denied distinct outcome has one root identity bound to current actor, case, purpose, action, sanitized subject, and result. |
| Replay, correction, and recovery | Replay is duplicate-safe; corrections and recovery milestones are immutable linked events; originals are never rewritten. |

Compliance export additionally requires a current approved, versioned, purpose-bound field-allowlist
policy compatible with the canonical prohibited-data taxonomy. If that policy is missing, stale, or
mismatched to the actor, case, purpose, or export version, the export fails closed without target
disclosure. The exact allowlist is an issuer policy dependency, not an implementation-time guess.

## Low-Level Task Agent and Delivery Matrix

Acceptance criteria are the `Done when` clauses in each task above. Paths below are hypothetical future
outputs only; no such source or test file is created by Homework 3.

| Task | Agent / tier | Least-cost rationale | Exact inputs | Hypothetical output | Dependencies | Parallel-safe | Independent reviewer |
|------|--------------|----------------------|--------------|---------------------|--------------|---------------|----------------------|
| LL-US1-01 | Control engineer / Standard | Approved command boundary makes implementation bounded | FR-001–FR-005; control contract; data model | `src/card-controls/application/freeze-cardholder.ts`; `test/card-controls/freeze-cardholder.spec.ts` | Shared state/idempotency foundation | No — establishes freeze acceptance | Payments/risk / Advanced |
| LL-US1-02 | Reliability engineer / Standard | Timelines and result categories are fixed | LL-US1-01; FR-003, FR-006, FR-041; R-003 | `src/card-controls/adapters/freeze-propagation.ts`; `test/recovery/freeze-propagation.spec.ts` | LL-US1-01 | No — consumes accepted intent | Reliability / Standard; risk sign-off / Advanced |
| LL-US1-03 | QA engineer / Standard | Explicit fixtures and oracles bound the work | LL-US1-01–02; EC-001–EC-005, EC-015, EC-022, EC-024 | `test/journeys/cardholder-freeze.e2e-spec.ts`; evidence report | LL-US1-01–02 | No — validates completed US1 | FinTech QA / Advanced |
| LL-US2-01 | Control engineer / Standard | Composite-state rules are normative | FR-007–FR-011; control contract; data model | `src/card-controls/application/unfreeze-cardholder.ts`; `test/card-controls/unfreeze-cardholder.spec.ts` | Shared state foundation | Yes — separate module after shared foundation | Payments/risk / Advanced |
| LL-US2-02 | Security/content engineer / Standard | Approved categories and disclosure rules remove ambiguity | LL-US2-01; FR-008–FR-011, FR-040 | `src/card-controls/presentation/unfreeze-results.ts`; content evidence | LL-US2-01 | No — depends on command results | Security/privacy / Advanced |
| LL-US2-03 | QA engineer / Standard | State matrix has objective pass/fail cells | LL-US2-01–02; EC-002–EC-004, EC-006, EC-010, EC-015, EC-023–EC-024 | `test/journeys/cardholder-unfreeze.e2e-spec.ts`; evidence report | LL-US2-01–02 | No — validates completed US2 | FinTech QA / Advanced |
| LL-US3-01 | Payment-controls engineer / Standard | Complete set validation is explicit | FR-012–FR-021; control contract; data model | `src/card-controls/application/set-spending-limits.ts`; unit/integration tests | Shared money/version foundation | Yes — separate module after shared foundation | Payments/domain / Advanced |
| LL-US3-02 | Risk-policy engineer / Standard | Policy branches are already approved | LL-US3-01; FR-016–FR-017, FR-021 | `src/card-controls/domain/limit-change-policy.ts`; policy tests | LL-US3-01 | No — consumes limit-set semantics | Payments/risk / Advanced |
| LL-US3-03 | Payment-accounting engineer / Standard | Accounting rules are normative but intricate | LL-US3-01–02; NFR-014; limit-accounting contract | `src/card-controls/domain/limit-consumption.ts`; property tests | LL-US3-01–02 | No — shared accounting model | Payment accounting / Advanced |
| LL-US3-04 | Payment QA engineer / Standard | Fixtures are specified; review carries domain risk | LL-US3-01–03; EC-002–EC-003, EC-006–EC-010, EC-012–EC-013, EC-021, EC-024–EC-026 | `test/journeys/spending-limits.e2e-spec.ts`; reconciliation report | LL-US3-01–03 | No — validates completed US3 | Payments/risk / Advanced |
| LL-US4-01 | History-query engineer / Standard | Field/ownership contract is explicit | FR-022–FR-025, FR-028, FR-030; history contract | `src/transaction-history/application/query-owned-history.ts`; contract tests | Shared authorization/taxonomy foundation | Yes — independent read module | Privacy/data / Advanced |
| LL-US4-02 | Data-query engineer / Standard | Snapshot/keyset decision is approved | LL-US4-01; FR-026–FR-028; R-004 | `src/transaction-history/domain/continuation.ts`; paging tests | LL-US4-01 | No — builds query continuation | Data/reliability / Standard |
| LL-US4-03 | Reliability/content engineer / Standard | Freshness thresholds are measurable | LL-US4-01–02; FR-029; EC-014 | `src/transaction-history/domain/freshness.ts`; classification tests | LL-US4-01–02 | No — completes query results | Reliability / Standard |
| LL-US4-04 | Data QA engineer / Standard | Scale and privacy oracles are fixed | LL-US4-01–03; EC-001, EC-011–EC-015, EC-022, EC-026 | `test/journeys/transaction-history.e2e-spec.ts`; traversal/privacy report | LL-US4-01–03 | No — validates completed US4 | Privacy/performance / Advanced |
| LL-US5-01 | Authorization engineer / Standard | Matrix is approved; reviewer owns high-risk judgment | FR-031–FR-032, FR-034–FR-037; internal-access contract | `src/internal-access/application/authorize-case-action.ts`; negative-matrix tests | Shared identity/case foundation | Yes — independent internal module | Security/compliance / Advanced |
| LL-US5-02 | Operations-risk engineer / Standard | Emergency behavior and thresholds are fixed | LL-US5-01; FR-031–FR-034, FR-037, FR-041 | `src/internal-access/application/add-operations-freeze.ts`; abuse/recovery tests | LL-US5-01; shared freeze boundary | No — consumes authorization and freeze invariants | Fraud/risk / Advanced |
| LL-US5-03 | Compliance engineer / Standard | Export is bounded by approved policy dependency | LL-US5-01; FR-035–FR-040; current versioned field allowlist | `src/internal-access/application/export-case-evidence.ts`; export-policy tests | LL-US5-01; approved allowlist available | No — fails closed on policy dependency | Compliance/privacy / Advanced |
| LL-US5-04 | Audit/reliability engineer / Standard | Evidence matrix and recovery timeline are explicit | Outcome-to-evidence matrix; FR-037–FR-041; R-002/R-005 | `src/audit/application/reconcile-evidence.ts`; replay/recovery tests | Control and privileged outcome producers | No — cross-cutting reconciliation | Security/compliance / Advanced |
| LL-US5-05 | Security/compliance QA / Standard | Negative matrix and timelines are explicit | LL-US5-01–04; EC-004–EC-006, EC-015–EC-020, EC-023–EC-024 | `test/journeys/internal-oversight.e2e-spec.ts`; assurance report | LL-US5-01–04 | No — validates completed US5 | Separate security/compliance / Advanced |
