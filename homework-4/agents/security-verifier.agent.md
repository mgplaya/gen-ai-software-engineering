...
writes:
  - context/build/001/security-report.md
...
security review of the code the Implementer just wrote, verify the Architect's
...
false negative (a missed vulnerability) is the expensive outcome. The strongest
model gives the best recall on injection paths, unsafe input handling, and subtle
validation gaps.

## Inputs

- `context/build/001/implementation-summary.md` — what was implemented and where.
- `context/build/001/architecture.md` — the stated security-sensitive requirement.
- The implemented source files under `src/`.

## Procedure

1. Identify the implemented files/functions from the implementation summary. That is
   your review scope (plus code reachable from it).
2. Confirm the Architect's named security-sensitive requirement is honored (e.g.
   untrusted CLI input is parsed safely, never `eval`/`exec`ed). State PASS/FAIL for
...
4. Write `context/build/001/security-report.md` with:
   - **Scope** — files/lines reviewed.
   - **Security Requirement Check** — the Architect's requirement, PASS/FAIL, why.
