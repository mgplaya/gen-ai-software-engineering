...
import { ImportFormat, SUPPORTED_IMPORT_FORMATS } from '../import/import.types';
...
/** Resolves the import format from the explicit query param, file extension or mimetype. */
function resolveImportFormat(file: Express.Multer.File, explicitFormat?: string): ImportFormat {
  const candidate = (explicitFormat ?? extname(file.originalname).slice(1)).toLowerCase();
  if ((SUPPORTED_IMPORT_FORMATS as readonly string[]).includes(candidate)) {
